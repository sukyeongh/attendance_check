...
const Lecture = ({ lecture }) => {
  ...
  const getAttendanceData = async () => {
    if (cookies.userid !== undefined) {
      const data = (
        await axios.get(`http://localhost:3001/api/attendances/attend/${lecture.lectureid}`, {
          params: {
  ...
  const onDeleteAttendance = async () => {
    const data = await axios.delete(`http://localhost:3001/api/attendances/${lecture.lectureid}`, {
      params: {
        studentid: cookies.userid,
      },
    });
